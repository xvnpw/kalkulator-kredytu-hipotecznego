...
var rowsTest = calcHarmonogram(350000, 2010, 1, 30, 2, '6M', 'rowna');
...
var rowsMal = calcHarmonogram(350000, 2010, 1, 30, 2, '6M', 'malejaca');
...
// ---------------------------------------------------------------------------
// 7. Deflator skumulowany (CPI m/m, 12+ miesiecy)
// ---------------------------------------------------------------------------
group('7. Deflator skumulowany (m/m)');
assertClose(rowsTest[0].deflator, 1.0, 1e-10, 'Deflator m0 = 1');
var cpiM_2010_01 = getCpiMonthly(2010, 1);
var expFactor = 1 / (1 + cpiM_2010_01 / 100);
assertClose(rowsTest[1].deflator, expFactor, 1e-6, 'Deflator m1 = 1/(1+CPI m/m sty 2010)');
...
assertClose(rowsTest[12].deflator, expected12, 1e-6, 'Deflator po 12 m = iloczyn 12 czynnikow miesiecznych');

// ---------------------------------------------------------------------------
// 8. Sumy nominalne - spojnosc
// ---------------------------------------------------------------------------
group('8. Sumy nominalne');
...
// ---------------------------------------------------------------------------
// 9. Sumy realne i zysk inflacyjny
// ---------------------------------------------------------------------------
group('9. Sumy realne i zysk inflacyjny');
...
var rows10 = calcHarmonogram(350000, 2010, 1, 10, 2, '6M', 'rowna');
...
// ---------------------------------------------------------------------------
// 10. Dekompozycja czynnikow (factor analysis)
// ---------------------------------------------------------------------------
group('10. Dekompozycja czynnikow');
var rows0 = calcHarmonogram(350000, 2010, 1, 30, 0, '6M', 'rowna');
...
// ---------------------------------------------------------------------------
// 11. Interwaly fixingu WIBOR
// ---------------------------------------------------------------------------
group('11. Interwaly fixingu WIBOR');
...
// WIBOR 3M
var rows3m = calcHarmonogram(350000, 2010, 1, 30, 2, '3M', 'rowna');
...
// WIBOR 1M
var rows1m = calcHarmonogram(350000, 2010, 1, 30, 2, '1M', 'rowna');
assert(rows1m[0].isFix === true, '1M: m0 fixing');
assert(rows1m[1].isFix === true, '1M: m1 fixing');
assert(rows1m[2].isFix === true, '1M: m2 fixing');

// ---------------------------------------------------------------------------
// 12. Mapowanie miesiaca startowego na kalendarz
// ---------------------------------------------------------------------------
group('12. Mapowanie miesiaca startowego');
var rowsJul = calcHarmonogram(350000, 2010, 7, 5, 2, '6M', 'rowna');
...
// 13. Agregacja roczna
// ---------------------------------------------------------------------------
group('13. Agregacja roczna (aggregateYearly)');
...
// ---------------------------------------------------------------------------
// 14. Annualizacja CPI miesiecznego
// ---------------------------------------------------------------------------
group('14. annualizeMonthlyCpi');
var cpiM1 = 0.5;
...
// ---------------------------------------------------------------------------
// 15. Fallback na domyslne wartosci przyszlosci
// ---------------------------------------------------------------------------
group('15. Domyslne wartosci przyszlosci');
...
// ---------------------------------------------------------------------------
// 16. Ujemne odsetki realne
// ---------------------------------------------------------------------------
group('16. Ujemne odsetki realne (2021, niska marza, krotki kredyt)');
...
var rows2021 = calcHarmonogram(350000, 2021, 1, 5, 0.5, '6M', 'rowna');
...
// ---------------------------------------------------------------------------
// 17. Malejace: przeliczenie kapitalu przy fixingu
// ---------------------------------------------------------------------------
group('17. Malejace - przeliczenie przy fixingu');
var rm6 = calcHarmonogram(350000, 2010, 1, 10, 2, '6M', 'malejaca');
...
// ---------------------------------------------------------------------------
// 18. Spot-check danych historycznych (CPI m/m + WIBOR + wynagrodzenia)
// ---------------------------------------------------------------------------
group('18. Spot-check danych');
// Annualizowana CPI z danych m/m dla 2022 powinna byc > 10% (rok wysokiej inflacji)
var cpiAnnualized2022 = annualizeMonthlyCpiForYear(2022);
assert(cpiAnnualized2022 !== null, 'CPI m/m dla 2022 istnieje (pelny rok)');
assert(cpiAnnualized2022 > 10, 'Annualizowana CPI 2022 > 10% (got ' + cpiAnnualized2022.toFixed(2) + ')');
...
// ---------------------------------------------------------------------------
// 19. Srednie roczne WIBOR
// ---------------------------------------------------------------------------
group('19. Srednie roczne WIBOR');
...
// ---------------------------------------------------------------------------
// 20. Cross-check annuitetowej raty ze znanymi wartosciami
// ---------------------------------------------------------------------------
group('20. Cross-check annuitet');
...
// ---------------------------------------------------------------------------
// 22. Deflacja (CPI m/m ujemne)
// ---------------------------------------------------------------------------
group('22. Deflacja (ujemne CPI m/m)');
// W 2015 r. wiele miesiecy mialo ujemny CPI m/m -> deflator > 1 w tych miesiacach
var found_deflationary_month = null;
for (var mo = 1; mo <= 12; mo++) {
  if (CPI_MONTHLY['2015-' + String(mo).padStart(2, '0')] < 0) {
    found_deflationary_month = mo;
    break;
  }
}
assert(found_deflationary_month !== null, 'W 2015 r. znaleziono miesiac z ujemna inflacja m/m');
if (found_deflationary_month !== null) {
  var deflFactor = getMonthlyDeflatorFactor(2015, found_deflationary_month);
  assert(deflFactor > 1, 'Deflator > 1 przy deflacji (got ' + deflFactor.toFixed(6) + ')');
}

// ---------------------------------------------------------------------------
// 23. Krotki i dlugi kredyt - edge cases
// ---------------------------------------------------------------------------
group('23. Edge cases: rozne okresy');
var rows3y = calcHarmonogram(100000, 2010, 1, 3, 2, '3M', 'rowna');
assert(rows3y.length === 36, '3-letni kredyt: 36 wierszy');
assertClose(rows3y[35].saldo, 0, 1.0, 'Saldo koncowe 3y ~0');

var rows35y = calcHarmonogram(500000, 2005, 1, 35, 2, '6M', 'rowna');
assert(rows35y.length === 420, '35-letni kredyt: 420 wierszy');
assertClose(rows35y[419].saldo, 0, 1.0, 'Saldo koncowe 35y ~0');

// ---------------------------------------------------------------------------
// 24. Porownanie WIBOR 6M vs 3M
// ---------------------------------------------------------------------------
group('24. WIBOR 6M vs 3M - rozne harmonogramy');
var rows6m = calcHarmonogram(350000, 2010, 1, 30, 2, '6M', 'rowna');
var rows3mComp = calcHarmonogram(350000, 2010, 1, 30, 2, '3M', 'rowna');
var rows1mComp = calcHarmonogram(350000, 2010, 1, 30, 2, '1M', 'rowna');
...
// ---------------------------------------------------------------------------
// 25. Rozne miesiace startowe
// ---------------------------------------------------------------------------
group('25. Rozne miesiace startowe');
var rowsJan = calcHarmonogram(350000, 2010, 1, 10, 2, '6M', 'rowna');
var rowsOct = calcHarmonogram(350000, 2010, 10, 10, 2, '6M', 'rowna');
...
// ---------------------------------------------------------------------------
// 26. Identycznosc rata = odsetki + kapital w kazdym wierszu
// ---------------------------------------------------------------------------
group('26. rata = odsetki + kapital (kazdy wiersz)');
var allOk = true;
...
// ---------------------------------------------------------------------------
// 27. Monotoniczne saldo (annuitet)
// ---------------------------------------------------------------------------
group('27. Saldo monotoniczne (annuitet)');
var monoAnn = true;
...
// ---------------------------------------------------------------------------
// 28. Monotoniczne saldo (malejace)
// ---------------------------------------------------------------------------
group('28. Saldo monotoniczne (malejace)');
var monoMal = true;
...
// ---------------------------------------------------------------------------
// 29. Wysoka inflacja 2022
// ---------------------------------------------------------------------------
group('29. Wysoka inflacja 2022');
var rows2022 = calcHarmonogram(350000, 2022, 1, 30, 2, '6M', 'rowna');
...
// ---------------------------------------------------------------------------
// 30. Prowizja: spojnosc nominalna i realna
// ---------------------------------------------------------------------------
group('30. Prowizja (spelnienie rownosci)');
...
// ---------------------------------------------------------------------------
// 31. Werdykt - kierunek decyzji
// ---------------------------------------------------------------------------
group('31. Werdykt - logika kierunku');
...
// ---------------------------------------------------------------------------
// 32. Przejscie deflatora na granicy roku (m/m)
// ---------------------------------------------------------------------------
group('32. Granica roku - deflator (m/m)');
// Wiersz m=11 (grudzien 2010) -> m=12 (styczen 2011)
// Deflator[12] = Deflator[11] * factor(grudzien 2010) (factor uzywa CPI m/m roku m-1)
var deflator11 = rowsTest[11].deflator;
var deflator12 = rowsTest[12].deflator;
var actualFactor = deflator12 / deflator11;
var expectedFact = 1 / (1 + getCpiMonthly(2010, 12) / 100);
assertClose(actualFactor, expectedFact, 1e-6, 'Deflator m11->m12 uzywa CPI m/m grudzien 2010');

// ---------------------------------------------------------------------------
// 33. Tryb WIBOR 6M w harmonogramie
// ---------------------------------------------------------------------------
group('33. WIBOR 6M w harmonogramie');
var rows6mTest = calcHarmonogram(350000, 2010, 1, 5, 2, '6M', 'rowna');
...
// ---------------------------------------------------------------------------
// 34. Wskaznik przystepnosci (rata/wynagrodzenie)
// ---------------------------------------------------------------------------
group('34. Wskaznik przystepnosci (rata / wynagrodzenie)');
salarySource = 'average';
...
// ---------------------------------------------------------------------------
// 35. getWynagr - fallback i ekstrapolacja
// ---------------------------------------------------------------------------
group('35. getWynagr fallback/ekstrapolacja');
salarySource = 'average';
...
// ---------------------------------------------------------------------------
// 36. getMonthlyDeflatorFactor (tryb m/m)
// ---------------------------------------------------------------------------
group('36. getMonthlyDeflatorFactor');
var fM = getMonthlyDeflatorFactor(2010, 1);
assert(fM > 0 && fM < 2, 'Deflator miesieczny w rozsadnym zakresie');
assertClose(fM, 1 / (1 + getCpiMonthly(2010, 1) / 100), 1e-10,
  'Deflator m/m = 1 / (1 + CPI m/m)');

// ---------------------------------------------------------------------------
// 37. aggregateYearly - wynagrodzenia
// ---------------------------------------------------------------------------
group('37. aggregateYearly - pola wynagrodzeniowe');
salarySource = 'average';
...
// ---------------------------------------------------------------------------
// 38. Parsowanie liczb dziesietnych (locale input)
// ---------------------------------------------------------------------------
group('38. Parsowanie liczb dziesietnych (locale input)');
