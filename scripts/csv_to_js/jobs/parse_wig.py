...
DEFAULT_INPUT = SOURCE_CSV_DIR / "wig_m.csv"
DEFAULT_OUTPUT = PROJECT_ROOT / "data-wig.js"
...
def run(input_path: Path, output_path: Path) -> int:
    ...
    ensure_non_empty(data, "WIG_MONTHLY")
    ensure_month_keys_contiguous(data, "WIG_MONTHLY")
    write_js_object(
        output_path,
        "WIG_MONTHLY",
        data,
        comments=[
            "WIG - monthly close values",
            'Key: "YYYY-MM", value: index close at month end',
            "Source: sources/csv/wig_m.csv",
        ],
    )
    return len(data)
...
def main() -> int:
    args = parse_job_args("Parse WIG CSV into data-wig.js", DEFAULT_INPUT, DEFAULT_OUTPUT)
    count = run(args.input, args.output)
    print(f"[wig] wrote {count} records -> {args.output}")
    return 0
